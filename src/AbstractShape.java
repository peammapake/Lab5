...
public abstract class AbstractShape
{
	...
	protected BoundingBox shapeArea = null;
	...
	/**
	 * Check if the given coordinate from mouse click is within certain shape parameter
	...
	/**
	 * Method to calculate the boundary box of the given shape
	 * except circle, it needs different method.
	 */
	protected  void calBoundBox()
	{
		...
		double highestX = anchor.x;
		double highestY = anchor.y;

		/*Loop through vertices to get the highest and lowest one*/
		for(int i = 0; i < vertices.size(); i++)
		{
			if(lowestX > vertices.get(i).x)
				lowestX = vertices.get(i).x;
			if(lowestY > vertices.get(i).y)
				lowestY = vertices.get(i).y;
			if(highestX < vertices.get(i).x)
				highestX = vertices.get(i).x;
			if(highestY < vertices.get(i).y)
				highestY = vertices.get(i).y;
		}
		double width = highestX - lowestX;
		double height = highestY - lowestY;
		/*Add the boundary to the Bouding Box class for further usage*/
